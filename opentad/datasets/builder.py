...
class DynamicLengthDistributedSampler(DistributedSampler):
    """A DistributedSampler that re-reads len(dataset) at each epoch.

    This is needed because the dataset length can change when
    empty_clip_ratio causes a different number of empty clips to be
    included each epoch.
    """

    def __iter__(self):
        # Recompute num_samples from current dataset length
        self.num_samples = math.ceil(
            (len(self.dataset) - self.num_replicas + 1) / self.num_replicas
        ) if not self.drop_last else math.floor(
...
def build_dataloader(dataset, batch_size, rank, world_size, shuffle=False, drop_last=False, seed=0, **kwargs):
    sampler = DynamicLengthDistributedSampler(
        dataset,
