    ...
    def post_processing(self, predictions, metas, post_cfg, ext_cls, **kwargs):
        ...
        for i in range(len(metas)):  # processing each video
            ...
            if num_classes == 1:
                ...
                # ── FIX: use torch.long so label.item() returns int ──
            ...
            # Handle ext_cls being None, list, or callable ──
            if ext_cls is None:
                labels = [str(int(label.item())) for label in labels]
            elif isinstance(ext_cls, list):  # own classification results
