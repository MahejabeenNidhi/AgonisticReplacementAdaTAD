...
class MixtureCausalBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Flash Attention requires fp16 or bf16 — cast locally, keep rest in fp32
        input_dtype = qkv.dtype
        if qkv.dtype == torch.float32:
            qkv = qkv.to(torch.bfloat16)
        x_t = flash_attn_qkvpacked_func(qkv, causal=True)
        if x_t.dtype != input_dtype:
            x_t = x_t.to(input_dtype)
