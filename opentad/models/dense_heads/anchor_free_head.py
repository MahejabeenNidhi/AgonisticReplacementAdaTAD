...
class AnchorFreeHead(nn.Module):
    ...
    def forward_test(self, feat_list, mask_list, **kwargs):
        ...
        points = self.prior_generator(feat_list)
        proposals, scores = self.get_valid_proposals_scores(points, reg_pred, cls_pred, mask_list)
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels):
        ...
        if self.training:
            ...
            loss_normalizer = max(self.loss_normalizer.item(), 5.0)  # floor
        else:
            loss_normalizer = max(num_pos, 1)

        # 1. classification loss
        cls_pred = [x.permute(0, 2, 1) for x in cls_pred]  # list of (B, T_l, C)
        cls_pred = torch.cat(cls_pred, dim=1)[valid_mask]  # (num_valid, C)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            cls_loss = self.cls_loss(cls_pred.float(), gt_target.float(), reduction="sum")
        ...
        gt_segments_reg = self.get_refined_proposals(points, gt_reg)[pos_mask]

        if num_pos == 0:
            reg_loss = pred_segments.sum() * 0
        else:
            with torch.cuda.amp.autocast(enabled=False):
                reg_loss = self.reg_loss(pred_segments.float(), gt_segments_reg.float(), reduction="sum")
