...
class TriDetHead(AnchorFreeHead):
    ...
    def losses(self, cls_pred, reg_pred, mask_list, points, gt_segments, gt_labels, out_start, out_end):
        ...
        # ── RECALL LOSS: capture full logits BEFORE valid_mask filtering ──
        if self.use_recall_loss:
            cls_logits_full = torch.cat(cls_pred, dim=1).float()  # (B, T_total, C)
            concat_points = torch.cat(points, dim=0).detach()  # (T_total, 4)
        # ──────────────────────────────────────────────────────────────────
        ...
        points_expanded = torch.cat(points, dim=0).unsqueeze(0).repeat(pos_mask.shape[0], 1, 1)
        points_expanded = points_expanded[pos_mask][torch.where(gt_cls[pos_mask])[0]]

        ## couple the classification loss with iou score
        pred_segments = self.get_proposals(points_expanded, pred_offsets)
        gt_segments_reg = self.get_proposals(points_expanded, gt_reg)

        iou_rate = self.iou_loss(pred_segments, gt_segments_reg, reduction="none")
        ...
        if num_pos == 0:
            ...
            reg_loss = self.reg_loss(pred_segments, gt_segments_reg, reduction="sum")
        ...
        losses_dict = {"cls_loss": cls_loss, "reg_loss": reg_loss * loss_weight}

        # ── 3. EVENT RECALL LOSS ─────────────────────────────────────────
        if self.use_recall_loss:
            has_gt_events = any(gt.shape[0] > 0 for gt in gt_segments)
            if has_gt_events:
                # Compute entirely in float32 to avoid AMP-induced overflow
                with torch.cuda.amp.autocast(enabled=False):
                    recall_loss = self.recall_loss(
                        cls_logits=cls_logits_full,  # already float32
                ...
                if torch.isnan(recall_loss) or torch.isinf(recall_loss):
                    recall_loss = torch.zeros(1, device=cls_loss.device, dtype=cls_loss.dtype, requires_grad=False)
                    recall_loss = recall_loss.squeeze() + cls_pred.sum() * 0.0  # maintain grad graph
            else:
                recall_loss = cls_pred.sum() * 0.0
            losses_dict["recall_loss"] = recall_loss
        # ──────────────────────────────────────────────────────────────────
